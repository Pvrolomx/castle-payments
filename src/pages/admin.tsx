...
export default function Admin({ initialInvoices, isConfigured }: Props) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        sessionStorage.setItem('castle_admin', 'true');
      } else {
  ...
  const getPaymentLink = (invoice: Invoice) => {
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://invoice.castlesolutions.mx';
  ...
  const logout = () => {
    sessionStorage.removeItem('castle_admin');
    setAuthenticated(false);
  };

  useEffect(() => {
    if (sessionStorage.getItem('castle_admin') === 'true') {
      setAuthenticated(true);
    }
  }, []);
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center px-4">
        <form onSubmit={handleLogin} className="bg-white p-8 rounded-lg shadow-sm border border-stone-200 w-full max-w-xs">
          <h1 className="text-center text-xl font-light tracking-wide text-stone-800 mb-6">Castle Admin</h1>
          {!isConfigured && (
    ...
    <div className="min-h-screen bg-stone-50 py-8 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-light tracking-wide text-stone-800">CASTLE ADMIN</h1>
            {!isConfigured && (
