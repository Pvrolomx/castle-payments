...
export default function Home() {
  ...
  const STRIPE_LINK = 'https://buy.stripe.com/PENDIENTE_STRIPE_LINK';
  ...
  const handleDownloadPDF = async () => {
    ...
    const invoiceHTML = `
      <div style="font-family: Georgia, serif; padding: 40px; max-width: 600px;">
        ...
        <div style="margin-bottom: 30px;">
          ...
          <p style="font-size: 16px; color: #555; margin: 0;">\${services || 'Real Estate Services'}</p>
        </div>
        ...
        <div style="margin-top: 50px; padding-top: 30px; border-top: 1px solid #e5e5e5;">
          ...
          <p style="font-size: 14px; color: #555; margin: 0;">invoice.castlesolutions.mx</p>
        </div>
    ...
    const opt = {
      margin: 0.5,
      filename: `invoice-\${client?.toString().toLowerCase().replace(/\s+/g, '-') || 'castle'}.pdf`,
            ...
            {showWire && (
              <div className="mt-4 text-xs text-stone-500 space-y-3 font-mono">
                <div>
                  ...
                  <p className="text-stone-700">[PENDIENTE - BENEFICIARIO]</p>
                  <p>[PENDIENTE - DIRECCION]</p>
                  <p>Puerto Vallarta, Jalisco</p>
                </div>
                <div>
                  <p className="text-stone-400 uppercase tracking-wider mb-1">Bank</p>
                  <p className="text-stone-700">[PENDIENTE - BANCO]</p>
                  <p>[PENDIENTE - DIRECCION BANCO]</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-stone-400 uppercase tracking-wider mb-1">CLABE</p>
                    <p className="text-stone-700">[PENDIENTE]</p>
                  </div>
                  <div>
